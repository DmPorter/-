...
void DeleteK(Table* T, Item* delitem, int key1) {
    ...
    printf("count = %d, size = %d.\n", T->count, T->size);
        ...
        for (i = 0; i < SIZE; i++) {     //проходим по массиву
            ...
            if (T->arr1[i].arr != NULL && T->arr1[i].arr != delitem) {
                printf("%5d  %8s %3d\n", T->arr1[i].arr->key1, findInfo1(T, i), T->arr1[i].busy);
            }
            else {
                printf("%5d      (null)%3d\n", delitem->key1, T->arr1[i].busy);
            }
        }
